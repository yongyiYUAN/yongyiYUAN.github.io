...
function preload() {
    ...
    // 加载字体
    digitalFont = loadFont('DS-DIGI-1.ttf');
    // 加载开始界面图片
...
function draw() {
    if (!gameStarted) {
        ...
        fill(48, 23, 3);
        textSize(30);
        text(`Score: ${score}`, width / 2 - 50, height / 2 + 35);
        text(`Time: ${finalTime}`, width / 2 - 50, height / 2 + 65);
    } else {
...
function checkGameOver() {
    if (lungVisible.every(v => v === false)) {
        gameOver = true;
        // 保存游戏结束时的时间
        finalTime = formatTime(millis() - startTime);
    }
}
...
function generateNewViruses(row) {
    ...
    if (timeSinceLastSpawn >= 5000) { // 调整时间间隔为 * 秒
        let virusesInRow = virusRows[row].viruses.length;

        if (virusesInRow < maxVirusesPerRow) {
            let numVirusesToGenerate = Math.min(
                maxVirusesToGenerate,
                maxVirusesPerRow - virusesInRow
            );

            for (let i = 0; i < numVirusesToGenerate; i++) {
                let virusIndex = floor(random(virusImages.length));
                let speed = random(1, 1.2);
                let delay = frameCount + i * 30;
